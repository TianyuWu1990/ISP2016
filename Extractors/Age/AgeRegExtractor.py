...
class AgeRegExtractor(object):
    ...
    def findEntity(self):
        ...
        extract_age = re.search(r'.*\s([0-9]+).?(years-old|year-old|yr|yrs|years|year|yo|y.o.|y.o|y/o).*', self.Text,
                                re.IGNORECASE)
        if not extract_age:
            extract_age = re.search(r'.*\s([0-9]+).?(months|months-old|months old|month-old|month old).*', self.Text,
                                    re.IGNORECASE)
            if not extract_age:
                age = "UNK"
                ageCode = "UNK"
        ...
        if extract_age:
            ...
            age_set = open('HTML_info/age.txt', 'w')
            age_set_str = str(age)
            age_offset_str = str(age_offset)
            age_set.write(age_set_str+age_offset_str)
            return [AgeElement(age, [age_offset], "AgeRegExtrator", "AGE"),
                    AgeCodeElement(ageCode, [ageCode_offset], "AgeRegExtrator", "AGE_COD")]
