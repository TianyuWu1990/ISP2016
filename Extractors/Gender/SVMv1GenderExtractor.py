...
class SVMv1GenderExtractor(object):
    ...
    def findEntity(self):
        ...
        for label in predictions:
            if label == 1:
                ...
                Genders.append(self.vectors[offset][0][0])
                Offsets.append([self.vectors[offset][0][1], self.vectors[offset][0][2]])
            offset += 1
        # print("SVMv1_gender:", Genders[0], Offsets[0])
        gender_set = open('HTML_info/gender.txt', 'w')
        print Genders
        gender_set_str = str(Genders)
        gender_set.write(gender_set_str + str(Offsets))
