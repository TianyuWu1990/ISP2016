...
class WeightRegExtractor(object):
    ...
    def findEntity(self):
        ...
        if not extract_weight:
            extract_weight = re.search(r'.*\s([0-9]+(\.[0-9]+)?).?(kg|kgs|kilograms|kilogram).*', self.Text,
                                       re.IGNORECASE)
            if not extract_weight:
                weight = "UNK"
            else:
        ...
        if extract_weight:
            ...
            weight_set = open('HTML_info/weight.txt', 'w')
            weight_set_str = str(weight)
            print weight_offset
            weight_set.write(weight_set_str+str(weight_offset))
            return [WeightElement(weight, [weight_offset], "WeightRegExtrator", "WT"),
                    WeightCodeElement(weightCode, [weightCode_offset], "WeightRegExtrator", "WT_COD")]
